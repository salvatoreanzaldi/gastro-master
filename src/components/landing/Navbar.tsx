...
const navKeys = ["produkte", "rechner", "referenzen", "preise", "faq"] as const;
const hrefs   = ["#produkte", "#rechner", "#referenzen", "#preise", "#faq"];
    ...
    <nav className={`fixed z-50 transition-all duration-700 ease-out rounded-2xl border border-primary-foreground/10 py-3 ${scrolled
      ...
      <div className="flex items-center justify-between px-4 md:px-5">
        ...
        <div className={`hidden md:flex items-center transition-all duration-700 ${scrolled ? "gap-4" : "gap-7"}`}>
          {navKeys.map((key, i) => (
            <a key={key} href={hrefs[i]} className="text-primary-foreground/70 hover:text-primary-foreground font-medium transition-all duration-500 text-sm">
              {t.nav[key]}
            </a>
          ))}
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-primary-foreground/10 px-5 py-5 mt-2 space-y-3">
          {navKeys.map((key, i) => (
            <a key={key} href={hrefs[i]} onClick={() => setMobileOpen(false)} className="block text-primary-foreground/70 hover:text-primary-foreground font-medium py-2">
              {t.nav[key]}
            </a>
          ))}
          <button onClick={scrollToForm}
