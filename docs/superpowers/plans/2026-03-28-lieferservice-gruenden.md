# Plan: LieferserviceGruendenPage — Vollständiger Overhaul

**Goal:** `src/pages/LieferserviceGruendenPage.tsx` von einem 50-Zeilen-Stub zu einer vollständigen ~700-Zeilen Ratgeber-Seite für Lieferdienst-Gründer überarbeiten.
**Architecture:** Self-contained single-file page (kein Component-Split). Folgt dem Muster von `TransaktionsumlagePage.tsx`. Nur diese eine Datei wird geändert.
**Tech stack:** React 18, TypeScript, Tailwind CSS, framer-motion (motion, AnimatePresence), lucide-react, react-router-dom (Link).
...
| Datei | Änderung |
|-------|----------|
| `src/pages/LieferserviceGruendenPage.tsx` | Vollständig ersetzt |
| `src/test/lieferservice-gruenden.test.ts` | Neu — Vitest-Tests für Daten-Integrität |

**Alle anderen Dateien bleiben unberührt.** Route (`App.tsx`) und Navbar-Link (`Navbar.tsx`) existieren bereits.

---

## Task 1: Daten-Layer + Vitest-Tests

**Ziel:** Alle TypeScript-Interfaces und Daten-Arrays schreiben, mit Tests validieren, bevor eine einzige JSX-Zeile existiert.

**Betroffene Dateien:**
- `src/test/lieferservice-gruenden.test.ts` — neu
- `src/pages/LieferserviceGruendenPage.tsx` — ersetzt Stub durch Daten-Layer

**Schritte:**

1. Test-Datei `src/test/lieferservice-gruenden.test.ts` erstellen:

```typescript
import { describe, it, expect } from "vitest";

// Die exports werden in Task 1 angelegt — vorerst leere Shapes testen
...
2. Test ausführen — erwartet Fehler (Exports existieren noch nicht):
```bash
cd "/Users/salvatore/Desktop/Gastro Master Dev/gastro-direct-launch"
npm test src/test/lieferservice-gruenden.test.ts
# Erwartet: FAIL — SyntaxError oder "does not export STATS"
```

3. `src/pages/LieferserviceGruendenPage.tsx` mit Daten-Layer ersetzen (noch kein JSX-Output, nur Exports + minimaler Stub-Component):
...
// ─── Page Component (Stub — wird in Tasks 2–7 befüllt) ───────────────────────
...
4. Tests ausführen — erwartet PASS:
```bash
npm test src/test/lieferservice-gruenden.test.ts
# Erwartet: PASS — 4/4 tests passing
```

5. Build-Check:
```bash
npm run build
# Erwartet: ✓ built in ~Xs (kein TypeScript-Fehler)
```

6. Commit:
```bash
git add src/pages/LieferserviceGruendenPage.tsx src/test/lieferservice-gruenden.test.ts
git commit -m "feat: lieferservice-gruenden — data layer + vitest coverage"
```

---

## Task 2: S1 — Hero-Sektion

**Betroffene Dateien:**
- `src/pages/LieferserviceGruendenPage.tsx` — Hero-JSX hinzufügen

**Schritte:**

1. Den Stub-Component in `LieferserviceGruendenPage.tsx` erweitern — `useEffect` für Meta-Tags + Hero-JSX:
...
```bash
npm run build
# Erwartet: ✓ built — kein Fehler
```

3. Commit:
```bash
git add src/pages/LieferserviceGruendenPage.tsx
git commit -m "feat: lieferservice-gruenden — S1 Hero section"
```

---

## Task 3: S2 — Marktstatistiken

**Betroffene Dateien:**
- `src/pages/LieferserviceGruendenPage.tsx` — Stats-Sektion nach Hero einfügen

**Schritte:**

1. Nach dem Hero-`</section>` und vor `{/* Remaining sections */}` einfügen:
...
```bash
git add src/pages/LieferserviceGruendenPage.tsx
git commit -m "feat: lieferservice-gruenden — S2 Marktstatistiken"
```

---

## Task 4: S3 — Schritt-für-Schritt Gründungsanleitung

**Betroffene Dateien:**
- `src/pages/LieferserviceGruendenPage.tsx` — Steps-Sektion einfügen

**Schritte:**

1. Nach der Stats-Sektion einfügen (mit `id="steps"` für den Anker-Link aus dem Hero):
...
```bash
git add src/pages/LieferserviceGruendenPage.tsx
git commit -m "feat: lieferservice-gruenden — S3 Schritt-für-Schritt"
```

---

## Task 5: S4 — Vergleichstabelle

**Betroffene Dateien:**
- `src/pages/LieferserviceGruendenPage.tsx` — Vergleichstabelle einfügen

**Schritte:**

1. Nach der Steps-Sektion einfügen:
...
```bash
git add src/pages/LieferserviceGruendenPage.tsx
git commit -m "feat: lieferservice-gruenden — S4 Vergleichstabelle"
```

---

## Task 6: S5 — FAQ-Akkordeon

**Betroffene Dateien:**
- `src/pages/LieferserviceGruendenPage.tsx` — FAQ-Sektion einfügen

**Hinweis:** `openFaq` State und `useState` Import wurden bereits in Task 2 angelegt.

**Schritte:**

1. Nach der Vergleichstabellen-Sektion einfügen:
...
```bash
git add src/pages/LieferserviceGruendenPage.tsx
git commit -m "feat: lieferservice-gruenden — S5 FAQ Accordion"
```

---

## Task 7: S6 — CTA-Abschluss + Build-Verifikation

**Betroffene Dateien:**
- `src/pages/LieferserviceGruendenPage.tsx` — CTA-Sektion + `{/* Remaining sections */}` Platzhalter entfernen

**Schritte:**

1. Nach dem FAQ `</section>` und vor `<Footer />` einfügen, und den `{/* Remaining sections — Tasks 3–7 */}` Kommentar entfernen:
...
2. Finaler Build-Check:
```bash
npm run build
# Erwartet: ✓ built in ~Xs — kein TypeScript- oder Lint-Fehler
```

3. Alle Vitest-Tests ausführen:
```bash
npm test
# Erwartet: PASS — alle Tests grün, inkl. lieferservice-gruenden.test.ts
```

4. Manuell prüfen (Dev-Server):
```bash
npm run dev
# Browser: http://localhost:8080/loesungen/lieferservice-gruenden
```

   Checkliste:
   - [ ] Alle 6 Sektionen sichtbar
   - [ ] Breadcrumb zeigt "Lösungen → Lieferdienst gründen"
   - [ ] Quellenlinks öffnen externen Tab
   - [ ] Produktlink-Chips in Schritt 4 navigieren korrekt
   - [ ] FAQ-Akkordeon: öffnen/schließen, immer nur eines offen
   - [ ] Vergleichstabelle scrollt auf Mobile horizontal
   - [ ] `document.title` korrekt (DevTools prüfen)
   - [ ] 2× JSON-LD Scripts vorhanden: `document.querySelectorAll('script[type="application/ld+json"]').length === 2`
   - [ ] Desktop-Styles anderer Seiten unverändert

5. Final Commit:
```bash
git add src/pages/LieferserviceGruendenPage.tsx
git commit -m "feat: lieferservice-gruenden — S6 CTA, vollständige Seite"
```
